...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!em.includes('@')) return toast.error('Enter a valid email.')
      ...
      <div className="w-full max-w-sm space-y-5 relative">
        ...
        <div className="text-center space-y-1.5">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl border border-accent-cyan/30 bg-accent-cyan/10 mb-2">
            <span className="text-accent-cyan font-mono font-bold text-xl">TP</span>
          </div>
          <h1 className="text-2xl font-bold text-slate-100 tracking-tight">TradePulse</h1>
          <p className="text-sm text-slate-500">Real-time stock dashboard with virtual trading.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            {tab === 'register' && (
              <div className="glass rounded-lg p-3 text-xs text-slate-400 font-mono space-y-0.5">
                <p>🎁 New accounts start with <span className="text-accent-gold font-semibold">$10,000</span> virtual balance</p>
